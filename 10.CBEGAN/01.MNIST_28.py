...
class Generator(torch.nn.Module):
    def __init__(self):
        ...
        self.model1 = torch.nn.Sequential(
            torch.nn.Linear(in_features=NOISE_DIM + NUM_CLASS,out_features=1024),
    ...
    def forward(self, x, label):
        network = torch.cat([x, label], dim=1)
        network = network.view(-1, NOISE_DIM + NUM_CLASS)

        network = self.model1(network)
...
class Discriminator(torch.nn.Module):
    def __init__(self):
        ...
        self.model1 = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=IMAGE_CHANNEL + NUM_CLASS, out_channels=64, kernel_size=4, stride=2, padding=1),
            torch.nn.ReLU()
        )
        self.model2 = torch.nn.Sequential(
...
dataset = epfd.MNISTDataSetForPytorch(root=DATA_PATH, transform=tv.transforms.Compose([
...
fix_noise = torch.FloatTensor(100, NOISE_DIM).view(-1, 100, 1, 1)
...
for epoch in range(1, EPOCH + 1):
    for index, (image, label) in enumerate(train_loader):
        ...
        noise = torch.rand(mini_batch, NOISE_DIM).view(-1, NOISE_DIM, 1, 1)
        label_var = torch.autograd.Variable(label.squeeze().type(torch.LongTensor))
    ...
    fake_u=NetG(fix_noise_var, fixed_y_label_)
