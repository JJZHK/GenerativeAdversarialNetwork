...
class StyleDiscriminator(torch.nn.Module):
    def __init__(self,
        ...
        # fromrgb: fixed mode
        self.fromrgb = torch.nn.Conv2d(num_channels, self.nf(self.resolution_log2-1), kernel_size=1)
        ...
        # down_sample
        self.down1  = torch.nn.AvgPool2d(2)
        self.down21 = torch.nn.Conv2d(self.nf(self.resolution_log2-5), self.nf(self.resolution_log2-5), kernel_size=2, stride=2)
        self.down22 = torch.nn.Conv2d(self.nf(self.resolution_log2-6), self.nf(self.resolution_log2-6), kernel_size=2, stride=2)
        self.down23 = torch.nn.Conv2d(self.nf(self.resolution_log2-7), self.nf(self.resolution_log2-7), kernel_size=2, stride=2)
        self.down24 = torch.nn.Conv2d(self.nf(self.resolution_log2-8), self.nf(self.resolution_log2-8), kernel_size=2, stride=2)

        # conv1: padding=same
        self.conv1 = torch.nn.Conv2d(self.nf(self.resolution_log2-1), self.nf(self.resolution_log2-1), kernel_size=3, padding=(1, 1))
        self.conv2 = torch.nn.Conv2d(self.nf(self.resolution_log2-1), self.nf(self.resolution_log2-2), kernel_size=3, padding=(1, 1))
        self.conv3 = torch.nn.Conv2d(self.nf(self.resolution_log2-2), self.nf(self.resolution_log2-3), kernel_size=3, padding=(1, 1))
        self.conv4 = torch.nn.Conv2d(self.nf(self.resolution_log2-3), self.nf(self.resolution_log2-4), kernel_size=3, padding=(1, 1))
        self.conv5 = torch.nn.Conv2d(self.nf(self.resolution_log2-4), self.nf(self.resolution_log2-5), kernel_size=3, padding=(1, 1))
        self.conv6 = torch.nn.Conv2d(self.nf(self.resolution_log2-5), self.nf(self.resolution_log2-6), kernel_size=3, padding=(1, 1))
        self.conv7 = torch.nn.Conv2d(self.nf(self.resolution_log2-6), self.nf(self.resolution_log2-7), kernel_size=3, padding=(1, 1))
        self.conv8 = torch.nn.Conv2d(self.nf(self.resolution_log2-7), self.nf(self.resolution_log2-8), kernel_size=3, padding=(1, 1))

        # calculate point:
        self.conv_last = torch.nn.Conv2d(self.nf(self.resolution_log2-8), self.nf(1), kernel_size=3, padding=(1, 1))
        self.dense0    = torch.nn.Linear(fmap_base, self.nf(0))
        self.dense1    = torch.nn.Linear(self.nf(0), 1)
        self.sigmoid   = torch.nn.Sigmoid()

    def forward(self, input):
        if self.structure == 'fixed':
            x = torch.nn.functional.leaky_relu(self.fromrgb(input), 0.2, inplace=True)
            # 1. 1024 x 1024 x nf(9)(16) -> 512 x 512
            res = self.resolution_log2
            x = torch.nn.functional.leaky_relu(self.conv1(x), 0.2, inplace=True)
            x = torch.nn.functional.leaky_relu(self.down1(self.blur2d(x)), 0.2, inplace=True)

            # 2. 512 x 512 -> 256 x 256
            res -= 1
            x = torch.nn.functional.leaky_relu(self.conv2(x), 0.2, inplace=True)
            x = torch.nn.functional.leaky_relu(self.down1(self.blur2d(x)), 0.2, inplace=True)

            # 3. 256 x 256 -> 128 x 128
            res -= 1
            x = torch.nn.functional.leaky_relu(self.conv3(x), 0.2, inplace=True)
            x = torch.nn.functional.leaky_relu(self.down1(self.blur2d(x)), 0.2, inplace=True)

            # 4. 128 x 128 -> 64 x 64
            res -= 1
            x = torch.nn.functional.leaky_relu(self.conv4(x), 0.2, inplace=True)
            x = torch.nn.functional.leaky_relu(self.down1(self.blur2d(x)), 0.2, inplace=True)

            # 5. 64 x 64 -> 32 x 32
            res -= 1
            x = torch.nn.functional.leaky_relu(self.conv5(x), 0.2, inplace=True)
            x = torch.nn.functional.leaky_relu(self.down21(self.blur2d(x)), 0.2, inplace=True)

            # 6. 32 x 32 -> 16 x 16
            res -= 1
            x = torch.nn.functional.leaky_relu(self.conv6(x), 0.2, inplace=True)
            x = torch.nn.functional.leaky_relu(self.down22(self.blur2d(x)), 0.2, inplace=True)

            # 7. 16 x 16 -> 8 x 8
            res -= 1
            x = torch.nn.functional.leaky_relu(self.conv7(x), 0.2, inplace=True)
            x = torch.nn.functional.leaky_relu(self.down23(self.blur2d(x)), 0.2, inplace=True)

            # 8. 8 x 8 -> 4 x 4
            res -= 1
            x = torch.nn.functional.leaky_relu(self.conv8(x), 0.2, inplace=True)
            x = torch.nn.functional.leaky_relu(self.down24(self.blur2d(x)), 0.2, inplace=True)

            # 9. 4 x 4 -> point
            x = torch.nn.functional.leaky_relu(self.conv_last(x), 0.2, inplace=True)
            # N x 8192(4 x 4 x nf(1)).
            x = x.view(x.size(0), -1)
            x = torch.nn.functional.leaky_relu(self.dense0(x), 0.2, inplace=True)
            # N x 1
            x = torch.nn.functional.leaky_relu(self.dense1(x), 0.2, inplace=True)
            return x
...
class G_synthesis(torch.nn.Module):
    def __init__(self,
        ...
        # Initial Input Block
        self.const_input = torch.nn.Parameter(torch.ones(1, self.nf(1), 4, 4))
        self.bias = torch.nn.Parameter(torch.ones(self.nf(1)))
...
class FC(torch.nn.Module):
    ...
    def forward(self, x):
        if self.bias is not None:
            out = torch.nn.functional.linear(x, self.weight * self.w_lrmul, self.bias * self.b_lrmul)
        else:
            out = torch.nn.functional.linear(x, self.weight * self.w_lrmul)
...
class Conv2d(torch.nn.Module):
    ...
    def forward(self, x):
        if self.bias is not None:
            return torch.nn.functional.conv2d(x, self.weight * self.w_lrmul, self.bias * self.b_lrmul, padding=self.kernel_size // 2)
        else:
            return torch.nn.functional.conv2d(x, self.weight * self.w_lrmul, padding=self.kernel_size // 2)


class Upscale2d(torch.nn.Module):
...
class LayerEpilogue(torch.nn.Module):
    def __init__(self,
                 channels,
                 dlatent_size,
                 use_wscale,
...
class GBlock(torch.nn.Module):
    def __init__(self,
        ...
        if res < 7:
            ...
            # upsample method 2
            self.up_sample = torch.nn.ConvTranspose2d(self.nf(res-3), self.nf(res-2), 4, stride=2, padding=1)
...
class ApplyStyle(torch.nn.Module):
    """
