...
DATA_PATH = "/data/input/stl10/train/"
EPOCHS = 100
BATCH_SIZE = 64
IMAGE_SIZE = 96
IMAGE_CHANNEL = 3
...
class Generator(torch.nn.Module):
    def __init__(self):
        ...
        self.deconv1 = torch.nn.Sequential(
            torch.nn.ConvTranspose2d(NOISE_DIM + NUM_CLASSES, 64 * 8, 4, 1, 0, bias=False),
...
class Discriminator(torch.nn.Module):
    def __init__(self):
        ...
        self.conv1 = torch.nn.Sequential(
            torch.nn.Conv2d(IMAGE_CHANNEL, 64, 4, 3, 1, bias=False),
            torch.nn.BatchNorm2d(64),
        ...
        self.conv5 = torch.nn.Sequential(
            torch.nn.Conv2d(64 * 8, 64 * 8, 4, 1, 0, bias=False)
        )

        self.fc1 = torch.nn.Sequential(
            torch.nn.Linear(512 + NUM_CLASSES, 256),
...
temp_z_ = torch.randn(NUM_CLASSES, 100)
fixed_z_ = temp_z_
Predict_y = torch.zeros(NUM_CLASSES, 1)
...
for epoch in range(1, EPOCHS + 1):
    ...
    tv.utils.save_image(test_images.data[:100],'outputs/STL96_%03d.png' % (epoch),
