...
class Generator(torch.nn.Module):
    def __init__(self):
        ...
        self.fc1 = torch.nn.Sequential(
            torch.nn.Linear(NOISE_DIM + NUM_CLASS, NOISE_DIM + NUM_CLASS)
        )

        self.deconv1 = torch.nn.Sequential(
            torch.nn.ConvTranspose2d(NOISE_DIM + NUM_CLASS, 64 * 16, 4, 1, 0, bias=False),
    ...
    def forward(self, input, label):
        ...
        x = self.fc1(x)
        x = x.view(-1, NOISE_DIM + NUM_CLASS, 1, 1)
...
class Discriminator(torch.nn.Module):
    def __init__(self):
        ...
        self.fc1 = torch.nn.Sequential(
            torch.nn.Linear(1024 + NUM_CLASS, 256),
...
def one_hot(target):
    y = torch.zeros(target.size()[0], NUM_CLASS)
...
predict_noise = torch.FloatTensor(NUM_CLASS, NOISE_DIM).normal_(0, 1)
...
for epoch in range(1, EPOCHS + 1):
    ...
    for img_real, label_real in dataLoader:
        ...
        label = one_hot(label_real.long().squeeze()-1)
    ...
    tv.utils.save_image(test_images.data[:NUM_CLASS],'outputs/Caltech256_%03d.png' % (epoch),
